 ...
 export const getOffchainAuthKeys = async (address: string, provider: any) => {
   ...
   if (storageResStr) {
     const storageRes = JSON.parse(
       CryptoJS.AES.decrypt(storageResStr, 'secret-key').toString(CryptoJS.enc.Utf8),
   ...
   localStorage.setItem(address, CryptoJS.AES.encrypt(JSON.stringify(offChainData), 'secret-key').toString());
   return offChainData;
 };
